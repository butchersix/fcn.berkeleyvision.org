...
def transplant(new_net, net, suffix=''):
    ...
    for p in net.params:
        ...
        if p_new not in new_net.params:
            # print 'dropping', p
            print('dropping {}'.format(p))
            continue
        for i in range(len(net.params[p])):
            if i > (len(new_net.params[p_new]) - 1):
                # print 'dropping', p, i
                print('dropping {} {}'.format(p, i))
                break
            if net.params[p][i].data.shape != new_net.params[p_new][i].data.shape:
                # print 'coercing', p, i, 'from', net.params[p][i].data.shape, 'to', new_net.params[p_new][i].data.shape
                print('coercing {} {} from {} to {}'.format(p, i, net.params[p][i].data.shape, new_net.params[p_new][i].data.shape))
            else:
                # print 'copying', p, ' -> ', p_new, i
                print('copying {} -> {} {}'.format(p, p_new, i))
...
def interp(net, layers):
    ...
    for l in layers:
        ...
        if m != k and k != 1:
            ...
            print('input + output channels need to be the same or |output| == 1')
            # raise
            raise Exception('input + output channels need to be the same or |output| == 1')
        if h != w:
