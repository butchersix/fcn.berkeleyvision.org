...
emotions_len = 3000
emotions_labeled_len = 635
emotions_unlabeled_len = emotions_len - emotions_labeled_len
threshold = 0.05
iteration_counter = 0
while(emotions_labeled_len < 3000):
    iteration_counter += 1
    emotions_labeled_len += 1 if int((threshold*emotions_unlabeled_len)) < 1 else int((threshold*emotions_unlabeled_len))
    emotions_unlabeled_len -= 1 if int((threshold*emotions_unlabeled_len)) < 1 else int((threshold*emotions_unlabeled_len))
...
# import pandas as pd
# emotions = pd.Series([1, 2, 3]
# print(len(emotions.loc[emotions == 1]))